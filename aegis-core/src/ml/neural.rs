...
impl Activation {
    ...
    pub fn apply(&self, x: f64) -> f64 {
        match self {
            ...
            Activation::Sigmoid => 1.0 / (1.0 + exp(-x.max(-500.0).min(500.0))),
            Activation::Tanh => {
                let e_pos = exp(x.max(-500.0).min(500.0));
                let e_neg = exp((-x).max(-500.0).min(500.0));
    ...
    pub fn apply_vec(&self, x: &[f64; MAX_NEURONS], len: usize) -> [f64; MAX_NEURONS] {
        ...
        match self {
            Activation::Softmax => {
                ...
                let mut sum = 0.0;
                for i in 0..len {
                    result[i] = exp((x[i] - max_val).max(-500.0).min(500.0));
                    sum += result[i];
                }
                for i in 0..len {
                    result[i] /= sum.max(1e-10);
                }
            },
            _ => {
                for i in 0..len {
...
impl DenseLayer {
    ...
    pub fn backward(&mut self, grad_output: &[f64; MAX_NEURONS], lr: f64) -> [f64; MAX_NEURONS] {
        ...
        for i in 0..self.output_size {
            for j in 0..self.input_size {
                // Update weight
                self.weights[i][j] -= lr * delta[i] * self.last_input[j];
                // Accumulate input gradient
                grad_input[j] += self.weights[i][j] * delta[i];
            }
            // Update bias
...
impl MLP {
    ...
    pub fn add_layer(&mut self, input_size: usize, output_size: usize, activation: Activation) {
        if self.n_layers >= MAX_LAYERS { return; }
    ...
    pub fn train_step(&mut self, input: &[f64; MAX_NEURONS], target: &[f64; MAX_NEURONS], output_size: usize) -> f64 {
        // Forward pass
...
pub fn adam_update(
    ...
    for i in 0..output_size {
        for j in 0..input_size {
            // Update moments
            state.m_weights[i][j] = params.beta1 * state.m_weights[i][j] + (1.0 - params.beta1) * grad_w[i][j];
            state.v_weights[i][j] = params.beta2 * state.v_weights[i][j] + (1.0 - params.beta2) * grad_w[i][j] * grad_w[i][j];
        ...
        state.v_biases[i] = params.beta2 * state.v_biases[i] + (1.0 - params.beta2) * grad_b[i] * grad_b[i];
...
pub fn sgd_momentum_update(
    ...
    for i in 0..output_size {
        for j in 0..input_size {
            state.velocity_weights[i][j] = momentum * state.velocity_weights[i][j] - lr * grad_w[i][j];
...
pub fn topo_regularization(outputs: &[[f64; MAX_NEURONS]], n_samples: usize, output_size: usize, epsilon: f64) -> f64 {
...
mod tests {
    ...
    fn test_mlp_xor() {
        ...
        let x = [
            {let mut a = [0.0; MAX_NEURONS]; a[0]=0.0; a[1]=0.0; a},
            {let mut a = [0.0; MAX_NEURONS]; a[0]=0.0; a[1]=1.0; a},
            {let mut a = [0.0; MAX_NEURONS]; a[0]=1.0; a[1]=0.0; a},
            {let mut a = [0.0; MAX_NEURONS]; a[0]=1.0; a[1]=1.0; a},
        ];
        let y = [
            {let mut a = [0.0; MAX_NEURONS]; a[0]=0.0; a},
            {let mut a = [0.0; MAX_NEURONS]; a[0]=1.0; a},
            {let mut a = [0.0; MAX_NEURONS]; a[0]=1.0; a},
            {let mut a = [0.0; MAX_NEURONS]; a[0]=0.0; a},
        ];
