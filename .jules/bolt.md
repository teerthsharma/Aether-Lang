...
## 2026-09-11 - Optimizing scalar reductions in linalg module
**Learning:** High-level mathematical functions (`mse`, `mae`, `binary_cross_entropy`, `hinge_loss`, `euclidean_distance`, `manhattan_distance`, `chebyshev_distance`, `rbf_kernel`) in the linalg module previously relied on `for` loops iterating over indices, leading to multiple bounds checks and potentially slow iterations in Hot Paths.
**Action:** Replaced manual index-based loops with iterator chains over borrowed data slices (`a_data.iter().zip(b_data.iter()).map(...).sum()`). This optimization ensures bounds checks are elided and enables LLVM auto-vectorization, directly targeting calculation speed for linear algebra primitives.
