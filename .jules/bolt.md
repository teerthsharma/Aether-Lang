...
## 2026-08-08 - Auto-vectorization of scalar reductions
**Learning:** Manual index-based loops (`for i in 0..n`) for scalar reductions in `aether-core::ml::linalg` prevent LLVM from eliding bounds checks and auto-vectorizing, creating a performance bottleneck for tensor operations.
**Action:** Use functional iterator chains (e.g., `.iter().zip().map().sum()`) directly over the underlying borrowed data arrays. When doing this, ensure inline conditional compilation attributes (`#[cfg]`) are preserved inside closures, and use `.fold` for maximums since `f64` lacks `Ord`.
