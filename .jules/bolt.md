...
## 2026-09-18 - Replacing loops with iterators for scalar reductions
**Learning:** In `aether-core::ml::linalg`, manual `for i in 0..n` loops for scalar reductions (e.g., `mse`, `mae`, `binary_cross_entropy`, `hinge_loss`, `euclidean_distance`, `manhattan_distance`, `chebyshev_distance`, `rbf_kernel`) do not allow LLVM to elide bounds checks or auto-vectorize as effectively as iterator chains.
**Action:** Replace manual indexing loops with single-pass iterator chains like `.iter().zip().map().sum()` (or `.fold()` when calculating maximums) over borrowed slice references to avoid overhead.
