...
## 2026-08-02 - Iterator usage in linear algebra scalar reductions
**Learning:** High-level scalar reduction operations in `ml::linalg` (e.g. `mse`, `mae`, distances) using manual bounds-checked loops (`for i in 0..n`) prevent auto-vectorization and have unnecessary overhead.
**Action:** Use single-pass iterators (`.iter().zip().map().sum()`) directly over the borrowed data arrays to elide bounds checks and allow LLVM to auto-vectorize effectively.
