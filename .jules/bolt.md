...
## 2026-08-01 - Optimizing Tensor reductions in linear algebra
**Learning:** In `aether-core::ml::linalg`, using manual `for i in 0..n` loops for scalar reductions (e.g. loss functions, distance metrics) relies on repeated array indexing which requires bounds checks.
**Action:** Replace indexed loops with functional iterator chains (`.iter().zip().map().sum()`) directly over the borrowed data arrays. This elides bounds checks and allows LLVM to auto-vectorize more effectively without intermediate allocations.
