...
## 2026-05-18 - Option::take() to Avoid Redundant Clone in Autograd Backward Pass
**Learning:** In the `aether-core::ml::autograd` reverse-mode `backward` pass, solving borrow checker constraints by simply calling `.clone()` on gradient options (`grads[out.index]`) resulted in unnecessary heap allocations of `Tensor` metadata (shape and strides vectors) for every operation.
**Action:** Use `Option::take()` to temporarily obtain ownership of complex structures from collections, process them by value to eliminate redundant clones, and re-insert them when done.
