...
## 2026-08-30 - Optimizing scalar reductions in linalg
**Learning:** Manual `for i in 0..n` index-based loops for scalar reductions (like `mse`, `mae`, and distances) over tensor data arrays require bounds checks that LLVM cannot easily elide, leading to suboptimal auto-vectorization.
**Action:** Replace manual loops with functional iterator chains (e.g., `.iter().zip().map().sum()`) directly over the underlying borrowed data arrays to enable bounds-check elision and optimal SIMD vectorization.
