...
## 2026-09-20 - Avoid allocation during tensor scalar reductions
**Learning:** In `aether-core::ml::linalg`, avoid high-level tensor operations like `a.sub(b)` and `.mul()` or explicit index-based loops when computing scalar reductions (e.g., `mse`, `mae`, etc.). They trigger costly intermediate heap allocations and hinder optimization.
**Action:** Perform a single-pass iteration directly over the underlying borrowed data arrays using functional iterator chains (e.g., `.iter().zip().map().sum()`). This elides bounds checks and allows LLVM to auto-vectorize more effectively. Ensure explicit `return` statements within these blocks are converted to implicit tail expressions to avoid `needless_return` clippy warnings.
