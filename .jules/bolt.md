...
## 2026-08-24 - Avoiding redundant slice allocations in neural backward
**Learning:** In DenseLayer::backward, generating the `dw_data` vector inside a loop and consuming it via `Tensor::new` causes unnecessary data cloning due to intermediate slice allocation.
**Action:** Replace `Tensor::new(&data, ...)` with `Tensor::from_vec(data, ...)` to consume the pre-allocated vector directly, preventing redundant O(N) memory allocations.
