...
## 2026-09-09 - Iterators for Scalar Reductions
**Learning:** Using `for i in 0..n` index-based loops for scalar reductions in `f64` slice computations incurs overhead due to bounds checking and impedes LLVM from auto-vectorizing operations efficiently.
**Action:** Replace manual loops with single-pass iterator chains like `.iter().zip().map().sum()` when reducing arrays to a single scalar value for better performance.
