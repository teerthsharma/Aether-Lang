...
## 2026-09-21 - Optimizing scalar reductions in linear algebra
**Learning:** High-level scalar reduction calculations in `linalg` like `.mse()` and `.binary_cross_entropy()` relying on manual explicit index-based `for` loops limit LLVM from auto-vectorizing and eliding slice bounds checks properly compared to using single-pass iterator chains (`.iter().zip().map().sum()`).
**Action:** Replace manual `for` loops doing scalar reductions across tensors with `.iter().zip().map().sum()` chains. Ensure `#[cfg]` attributes inside closures are maintained carefully.
