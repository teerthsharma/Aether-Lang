...
## 2026-08-14 - Bounds check elision in linear algebra scalar reductions
**Learning:** Manual index-based loops in tensor scalar reductions incur bounds checking overhead and inhibit LLVM auto-vectorization.
**Action:** Replace `for i in 0..n` with single-pass functional iterator chains (`.iter().zip().map().sum()`) directly over borrowed data arrays, and use `.fold()` for non-Ord reductions like `f64` maximums.
