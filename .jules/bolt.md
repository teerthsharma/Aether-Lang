...
## 2026-07-31 - Taking ownership of cached inputs in DenseLayer backward pass
**Learning:** In `aether-core`, `DenseLayer::backward` clones `last_z` and `last_input` from its cache, which causes unnecessary heap allocations for shape and stride metadata.
**Action:** Use `Option::take()` on `last_z` and `last_input` to obtain ownership, avoiding unnecessary metadata cloning while preventing mutable borrow conflicts during backpropagation.
