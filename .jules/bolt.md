...
## 2026-08-11 - Iterator Chains for LLVM Vectorization
**Learning:** Manual index-based loops for tensor operations like `mse` and `mae` introduce bounds checking and prevent effective LLVM vectorization.
**Action:** Replace `for i in 0..n` with `.iter().zip().map().sum()` to elide bounds checks and let LLVM auto-vectorize.
