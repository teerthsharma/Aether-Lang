...
## 2026-08-16 - Iterator Chains for LLVM Auto-vectorization in Linalg Reductions
**Learning:** In `aether-core::ml::linalg`, replacing manual `for i in 0..n` loops with functional iterator chains (`.iter().zip().map().sum()`) over `RefCell` borrowed slices elides bounds checks and allows LLVM to auto-vectorize more effectively, greatly speeding up scalar reductions like MSE and distance calculations.
**Action:** Always prefer functional iterator chains directly over underlying borrowed data arrays rather than manual index-based loops for scalar reductions. Use `.fold(0.0, |max_val, abs_val| ...)` when `f64` lacks `Ord` for `.max()`.
