...
## 2026-09-17 - Optimizing scalar reductions in linear algebra
**Learning:** In `aether-core::ml::linalg`, avoid high-level tensor operations like `a.sub(b)` and `.mul()` when computing scalar reductions (e.g., `mse`, `mae`, etc.) as they trigger costly intermediate heap allocations. Instead, perform a single-pass iteration directly over the underlying borrowed data arrays using functional iterator chains (e.g., `.iter().zip().map().sum()`). This elides bounds checks and allows LLVM to auto-vectorize more effectively compared to manual `for i in 0..n` index-based loops.
**Action:** Replace manual `for i in 0..n` index-based loops with functional iterator chains (`.iter().zip().map().sum()`) when implementing metrics or losses over contiguous tensor data.
