...
## 2026-08-15 - Scalar reductions and iterators
**Learning:** In `aether-core::ml::linalg`, scalar reductions (e.g., `mse`, `mae`) previously used manual index-based for-loops. While avoiding `.sub()` prevents high-level allocations, manual indexing bypasses LLVM's auto-vectorization capabilities.
**Action:** Always use single-pass iterator chains (`.iter().zip().map().sum()`) directly over borrowed tensor data to allow LLVM to elide bounds checks and vectorize reductions.
