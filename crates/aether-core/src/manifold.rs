...
impl<const D: usize> ManifoldPoint<D> {
    ...
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        // Optimization: Uses squared distance and early return to avoid expensive libm::sqrt
        // and redundant dimension calculations when points are already too far apart.
        if !(epsilon > 0.0) {
            return false; // Handle negative or NaN thresholds
        }
