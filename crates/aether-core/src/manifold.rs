...
impl<const D: usize> ManifoldPoint<D> {
    ...
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        // Optimization: Avoid `libm::sqrt` overhead by using squared distance.
        // Explicitly reject negative or NaN thresholds before the loop.
        if !(epsilon > 0.0) {
            return false;
        }
        ...
        for i in 0..D {
            ...
            // Early exit within the loop. Formatted as `!(sum < eps_sq)` to safely
            // handle `NaN` values without altering behavioral logic.
            if !(sum < eps_sq) {
                return false;
            }
        }
