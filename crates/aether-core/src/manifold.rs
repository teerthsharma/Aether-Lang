...
impl<const D: usize> ManifoldPoint<D> {
    ...
    /// Optimized to avoid expensive `libm::sqrt` calls by using squared
    /// distances, with early exit condition handling NaN safety correctly.
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        ...
        for i in 0..D {
            ...
            // Early exit if distance already exceeds epsilon squared
            // The !(sum < eps_sq) handles NaN propagation safely without false positives
            if !(sum < eps_sq) {
                return false;
            }
        }
