...
impl<const D: usize> ManifoldPoint<D> {
    ...
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        // Optimization: Avoid expensive `libm::sqrt` in hot spatial scans.
        // Explicitly handle negative or NaN epsilon.
        if !(epsilon > 0.0) {
            return false;
        }
        ...
        for i in 0..D {
            ...
            // Early exit: Break loop if distance exceeds squared threshold.
            // Using `!(sum < eps_sq)` correctly handles NaN coordinates.
            if !(sum < eps_sq) {
                return false;
            }
        }
