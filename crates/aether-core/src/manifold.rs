...
impl<const D: usize> ManifoldPoint<D> {
    ...
    ///
    /// ⚡ Bolt Optimization:
    /// In hot spatial scans like `SparseAttentionGraph::add_point`, avoiding `libm::sqrt`
    /// and using early exits via squared distance comparisons (`d^2 < r^2`) yields a ~6x speedup.
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        ...
        for i in 0..D {
            let d = self.coords[i] - other.coords[i];
            sum += d * d;
            // Early exit if we exceed the squared threshold
            if sum >= eps_sq {
                return false;
            }
        }
