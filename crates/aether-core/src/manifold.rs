...
impl<const D: usize> ManifoldPoint<D> {
    ...
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        // Optimization: Avoid costly `libm::sqrt` in hot O(N) spatial scans.
        // Reject negative or NaN epsilons upfront.
        if !(epsilon > 0.0) {
            return false;
        }
        ...
        for i in 0..D {
            ...
            // Early exit if distance squared already exceeds epsilon squared.
            // Using `!(sum < eps_sq)` correctly propagates false if sum is NaN.
            if !(sum < eps_sq) {
                return false;
            }
        }
