...
impl<const D: usize> ManifoldPoint<D> {
    ...
    // ⚡ Bolt: Use squared distance to avoid `libm::sqrt` overhead in hot spatial scans.
    // Early exit inside the loop prevents unnecessary computations.
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        ...
        for i in 0..D {
            let d = self.coords[i] - other.coords[i];
            sum += d * d;
            // Handle NaN safely by formatting as `!(sum < eps_sq)`
            if !(sum < eps_sq) {
                return false;
            }
        }
