...
impl<const D: usize> ManifoldPoint<D> {
    ...
    ///
    /// **Optimization:** This implementation uses squared distance to avoid the overhead of `libm::sqrt`.
    /// It also includes an early return if the running sum of squared differences exceeds the squared epsilon.
    /// This provides significant speedups during dense neighbor search loops like `add_point`.
