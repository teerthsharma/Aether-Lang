...
impl<const D: usize> ManifoldPoint<D> {
    ...
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        ...
        // ⚡ Bolt: Optimize hot spatial scan by comparing squared distances
        // Avoids costly libm::sqrt overhead in O(N) neighbor checks during add_point.
        // Also includes an early exit to skip remaining dimensions if we already exceed radius.
        ...
        for i in 0..D {
            ...
            // Safe NaN handling: Use `!(sum < eps_sq)` instead of `sum >= eps_sq`
            if !(sum < eps_sq) {
                return false;
            }
        }
