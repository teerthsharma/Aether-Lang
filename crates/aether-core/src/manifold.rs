...
impl<const D: usize> ManifoldPoint<D> {
    ...
    /// Optimized to use squared distance to avoid libm::sqrt overhead.
    /// Safely handles NaN epsilon and coordinates using inverted checks.
    pub fn is_neighbor(&self, other: &Self, epsilon: f64) -> bool {
        ...
        for i in 0..D {
            let d = self.coords[i] - other.coords[i];
            sum += d * d;
            // Early exit condition (negated to handle NaN correctly)
            if !(sum < eps_sq) {
                return false;
            }
        }
