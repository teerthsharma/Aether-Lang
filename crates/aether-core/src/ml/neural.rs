...
impl DenseLayer {
    ...
    pub fn backward(&mut self, grad_output: &Tensor, config: &OptimizerConfig) -> Tensor {
        ...
        // Use optimized outer product for weight gradients
        let mut grad_w = delta.outer(last_input);
        // Ensure shape matches the expected weight shape [output_size, input_size]
        grad_w.shape = self.weights.shape.clone();
        grad_w.strides = self.weights.strides.clone();
