...
impl Tensor {
    ...
    /// Returns a 2D tensor where result[i, j] = self[i] * other[j]
    /// Optimized for fast backpropagation using mapped iterators
    pub fn outer(&self, other: &Tensor) -> Tensor {
        let data_self = self.data.borrow();
        let data_other = other.data.borrow();

        let m = data_self.len();
        let n = data_other.len();

        let mut result_data = Vec::with_capacity(m * n);

        // Use extend and map for better vectorization than manual nested loops
        for i in 0..m {
            let val_i = data_self[i];
            result_data.extend(data_other.iter().map(|&val_j| val_i * val_j));
        }

        Self::new(&result_data, &[m, n])
    }
