...
pub fn mse(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    assert_eq!(y_true.shape, y_pred.shape, "Shapes must match for mse");
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
    let n = true_data.len();
...
pub fn mae(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    assert_eq!(y_true.shape, y_pred.shape, "Shapes must match for mae");
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
    let n = true_data.len();
...
pub fn binary_cross_entropy(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    assert_eq!(
        ...
        "Shapes must match for binary_cross_entropy"
    );
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
...
pub fn hinge_loss(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    assert_eq!(
        y_true.shape, y_pred.shape,
        "Shapes must match for hinge_loss"
    );
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
...
pub fn euclidean_distance(a: &Tensor, b: &Tensor) -> f64 {
    assert_eq!(a.shape, b.shape, "Shapes must match for euclidean_distance");
    let mut sum = 0.0;
...
pub fn manhattan_distance(a: &Tensor, b: &Tensor) -> f64 {
    assert_eq!(a.shape, b.shape, "Shapes must match for manhattan_distance");
    let mut sum = 0.0;
    let a_data = a.data.borrow();
    let b_data = b.data.borrow();
    for i in 0..a_data.len() {
...
pub fn chebyshev_distance(a: &Tensor, b: &Tensor) -> f64 {
    assert_eq!(a.shape, b.shape, "Shapes must match for chebyshev_distance");
    let mut max = 0.0;
    let a_data = a.data.borrow();
    let b_data = b.data.borrow();
    for i in 0..a_data.len() {
...
pub fn rbf_kernel(a: &Tensor, b: &Tensor, gamma: f64) -> f64 {
    assert_eq!(a.shape, b.shape, "Shapes must match for rbf_kernel");
    let mut dist_sq = 0.0;
    let a_data = a.data.borrow();
    let b_data = b.data.borrow();
    for i in 0..a_data.len() {
