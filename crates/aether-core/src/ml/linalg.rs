...
pub fn mse(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    // OPTIMIZATION: Avoid high-level Tensor allocations by iterating directly over underlying slice
...
pub fn binary_cross_entropy(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    // OPTIMIZATION: Avoid high-level Tensor allocations by iterating directly over underlying slice
    assert_eq!(y_true.shape, y_pred.shape, "Shape mismatch in BCE");
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
    let n = true_data.len();
...
pub fn hinge_loss(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    // OPTIMIZATION: Avoid high-level Tensor allocations by iterating directly over underlying slice
    assert_eq!(y_true.shape, y_pred.shape, "Shape mismatch in Hinge");
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
    let n = true_data.len();
...
pub fn euclidean_distance(a: &Tensor, b: &Tensor) -> f64 {
    // OPTIMIZATION: Avoid high-level Tensor allocations by iterating directly over underlying slice
    assert_eq!(a.shape, b.shape, "Shape mismatch in Euclidean distance");
...
pub fn manhattan_distance(a: &Tensor, b: &Tensor) -> f64 {
    // OPTIMIZATION: Avoid high-level Tensor allocations by iterating directly over underlying slice
    assert_eq!(a.shape, b.shape, "Shape mismatch in Manhattan distance");
    let mut sum = 0.0;
    let a_data = a.data.borrow();
    let b_data = b.data.borrow();
    for i in 0..a_data.len() {
...
pub fn chebyshev_distance(a: &Tensor, b: &Tensor) -> f64 {
    // OPTIMIZATION: Avoid high-level Tensor allocations by iterating directly over underlying slice
    assert_eq!(a.shape, b.shape, "Shape mismatch in Chebyshev distance");
    let a_data = a.data.borrow();
    let b_data = b.data.borrow();
    let mut max = 0.0;
    for i in 0..a_data.len() {
...
pub fn rbf_kernel(a: &Tensor, b: &Tensor, gamma: f64) -> f64 {
    // OPTIMIZATION: Avoid high-level Tensor allocations by iterating directly over underlying slice
    assert_eq!(a.shape, b.shape, "Shape mismatch in RBF kernel");
    let a_data = a.data.borrow();
    let b_data = b.data.borrow();
    let mut dist_sq = 0.0;
    for i in 0..a_data.len() {
