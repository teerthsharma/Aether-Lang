...
pub fn mse(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    // BOLT OPTIMIZATION: Avoiding intermediate Tensor heap allocations.
    // Iterating over borrowed data directly instead of using `.sub()` and `.mul()`.
...
pub fn mae(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    // BOLT OPTIMIZATION: Single-pass over data array avoids allocating intermediate Tensor metadata
    assert_eq!(y_true.shape, y_pred.shape);
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
...
pub fn euclidean_distance(a: &Tensor, b: &Tensor) -> f64 {
    // BOLT OPTIMIZATION: Calculate directly over borrowed data to prevent heap allocation overhead.
