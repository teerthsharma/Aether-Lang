...
pub fn mse(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    ...
    // ⚡ Bolt Optimization: Functional iterator chain to elide bounds checks and auto-vectorize
...
pub fn chebyshev_distance(a: &Tensor, b: &Tensor) -> f64 {
    ...
    // ⚡ Bolt Optimization: Iterator chain with fold to elide bounds checks and handle f64 max
    a_data
        .iter()
        .zip(b_data.iter())
        .map(|(x, y)| fabs(x - y))
