...
pub fn mse(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    // OPTIMIZATION: Compute scalar reduction directly on borrowed data arrays
    // in a single pass instead of creating intermediate Tensors via sub/mul.
...
pub fn mae(y_true: &Tensor, y_pred: &Tensor) -> f64 {
    // OPTIMIZATION: Avoid `.min()` truncation; assert shape equality and iterate once.
    assert_eq!(y_true.shape, y_pred.shape);
    let mut sum = 0.0;
    let true_data = y_true.data.borrow();
    let pred_data = y_pred.data.borrow();
...
pub fn euclidean_distance(a: &Tensor, b: &Tensor) -> f64 {
    // OPTIMIZATION: Compute directly on borrowed data without allocating a difference Tensor.
